    ...
    try {
      ...
      items.forEach((item) => {
        ...
        if (
          ...
          // Get the recurring start time from the new field
          const recurringTimeText = $el.find(`#data-recurring-start-time`).text().trim();

          // Compute first occurrence DATE with the recurring time
      ...
      console.error('Events script error:', error);
      // Always show events even if there's an error
      $(`[data-type="events-col"]`).show();
      $(`[data-role="loading"]`).hide();
    }
  }
...
// Parse time string like "9:00 PM" or "5:30 AM"
function parseTimeString(timeStr) {
  if (!timeStr) return { hours: 0, minutes: 0 };

  const timeMatch = timeStr.match(/(\d{1,2}):(\d{2})\s*(AM|PM)/i);
  if (timeMatch) {
    let [, hh, min, ap] = timeMatch;
    hh = parseInt(hh, 10);
    min = parseInt(min, 10);
    ap = ap.toUpperCase();
...
// Compute the next recurring DATE and attach the recurring time
function getFirstRecurringDateTime(eventType, originalStartText, recurringTimeText) {
  const now = new Date();
  const original = parseDateTime(originalStartText);

  // Priority: Use recurringTimeText if provided, otherwise fall back to original time
  let baseHours = 0;
  let baseMinutes = 0;

  if (recurringTimeText) {
    const parsedTime = parseTimeString(recurringTimeText);
    baseHours = parsedTime.hours;
    baseMinutes = parsedTime.minutes;
  } else if (!isNaN(original)) {
    baseHours = original.getHours();
    baseMinutes = original.getMinutes();
  }
...
function generateMonthList(endDate) {
  ...
  while (true) {
    ...
    if (endDate) {
      ...
      if (
        !isNaN(end) && (currentYear > end.getFullYear() ||
