(function () {
  ...
  //   "weekly friday"
  //   "every friday"
  //   "monthly - 1, 15, 30" or "monthly 1st, 3rd"
  function parseRule(raw) {
    ...
    if (s.includes('weekly') || s.includes('every week') || s.includes('every ')) {
      ...
      // If nothing explicit, assume all 7 days which still gives a correct sort key for daily‑like weeklys
  ...
  function keyToTs(key) {
    ...
    // Using Date(y,m,d,h,mi) keeps this in local time which matches how the page shows times
  }
    ...
    const formatRange = (items, isDays = false) => {
      ...
      for (let i = 1; i < items.length; i++) {
        if (items[i] === prev + 1) {
          ...
          out.push(start === prev ? `${start}${end(start)}` : `${start}${end(start)} - ${prev}${end(prev)}`);
          start = items[i];
          prev = items[i];
        }
      }
